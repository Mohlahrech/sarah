...
class WebsiteSaleAddressCity(WebsiteSale):
    ...
    def state_infos(self, state, mode, **kw):
        if state:
            return dict(cities=[(ct.id, ct.name) for ct in state.get_website_sale_city(mode=mode)], )
    ...
    def address(self, **kw):
            ...
            if errors:
                ...
                if mode[1] == 'billing':
                    ...
                    if not kw.get('use_same'):
                        kw['callback'] = kw.get('callback') or (not order.only_services and (
        ...
        render_values = {
            ...
            # "cities": state.get_website_sale_city(mode=mode[1]),
            'error': errors,
